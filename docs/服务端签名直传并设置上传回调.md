您可以使用[PostObject](https://help.aliyun.com/zh/oss/developer-reference/postobject)接口将文件直接从 Web 端上传到 OSS，支持设置上传回调，服务器生成的签名为直传操作保障传输安全，同时支持配置上传策略（Policy）以限制上传操作并满足业务需求。

## **方案概览**

![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8650449771/CAEQVRiBgMDj0OaUqhkiIDU0ZjJjNGNkZWY3NDQ3MWVhM2Q4N2U5NDMxYzJlZjFh4749084_20241106202116.368.svg)

要实现服务端签名直传并设置上传回调，只需3步：

**说明**

由于使用了临时访问凭证，整个过程中不会泄露业务服务器的长期密钥，保证了文件上传的安全性。

1.  **配置OSS**：在控制台创建一个Bucket，用于存储用户上传的文件。同时，为 Bucket 配置跨域资源共享（CORS） 规则，以允许来自Web端的跨域请求。
    
2.  **配置服务端：**使用访问凭证和服务端预设的上传策略（如Bucket名称、目录路径、过期时间等）生成签名，授权用户在一定时间内进行文件上传。
    
3.  **配置Web端**：构造HTML表单并将文件直传到OSS，OSS解析客户端的上传回调设置，发送Post回调请求给应用服务器。
    

## 示例工程

-   Java完整示例工程请参见：[server-signed-direct-upload-callback-java.zip](https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/zh-CN/20250612/pfbigz/server-signed-direct-upload-callback-java.zip)
    
-   Python完整示例工程请参见：[server-signed-direct-upload-callback-python.zip](https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/zh-CN/20250324/ndgufh/server-signed-direct-upload-callback-python.zip)
    
-   Node.js完整示例工程请参见：[server-signed-direct-upload-callback-nodejs.zip](https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/zh-CN/20250227/fjlfne/server-signed-direct-upload-callback-nodejs.zip)
    
-   Go完整示例工程请参见：[server-signed-direct-upload-callback-go.zip](https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/zh-CN/20250324/bwdqaa/server-signed-direct-upload-+callback-go.zip)
    
-   PHP完整示例工程请参见：[server-signed-direct-upload-callback-php.zip](https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/zh-CN/20250227/czknwm/server-signed-direct-upload-callback-php.zip)
...
3.  在**创建 Bucket**面板，选择快捷创建，按如下说明配置各项参数。
    ...
    | **参数** | **示例值** |
    | --- | --- |
    | **Bucket名称** | web-direct-upload- callback |
    | **地域** | 华东1（杭州） |
...
在实际部署时，如果您已经有自己的业务服务器，则无需进行准备工作，直接跳转到[一、配置用户权限](https://help.aliyun.com/zh/oss/user-guide/obtain-signature-information-from-the-server-and-upload-data-to-oss#4d7efb1a2fbof)。
...
2.  **创建专有网络VPC & 交换机**
    ...
    **说明**
    ...
    在创建专有网络页面，设置**名称**为`ProjectA-VPC`，**IPv4网段**选择**手动输入 IPv4 地址段**并填写`192.168.0.0/16`，**IPv6网段**选择**不分配**。
    
    在交换机配置区域，设置**名称**为 `ProjectA-vSwitch`，**可用区**选择**杭州 可用区H**，**IPv4网段**设置为 `192.168.10.0/24`，然后单击**确定**完成创建。
...
3.  **选择规格 & 镜像**
    ...
    在**实例规格**区域，选择**所有规格**页签，搜索并选择`ecs.e-c1m1.large`规格。在**镜像**区域，选择**公共镜像**页签，操作系统选择**Alibaba Cloud Linux**，版本选择**3.2104 LTS 64位**。
...
4.  **选择存储**
    ...
    系统盘选择**ESSD云盘**，容量设置为**40 GiB**，云盘性能选择**PL0（单盘IOPS性能上限1万）**，勾选**随实例释放**。
...
7.  **创建密钥对**
    ...
    2.  `root`具有操作系统的最高权限，使用`root`作为登录名可能会导致安全风险，建议您选择`ecs-user`作为登录名。
        ...
        **说明**
        ...
        单击**创建密钥对**链接创建密钥对，然后在密钥对下拉框中选择已创建的密钥对（如 `ecs-test`）。
...
8.  **创建并查看ECS实例**
    ...
    在ECS控制台的实例列表中，您可以在对应列查看实例ID、地域和公网IP地址等信息。
...
您可以参考如下代码来完成服务端获取临时访问凭证，并计算POST签名和设置上传回调：
...
请参考如下代码来完成服务端获取临时访问凭证STStoken并构建上传策略以计算POST签名并设置回调。
...
def generate_upload_params():
    ...
    # 构建回调配置并 Base64 编码
    callback_config = {
        "callbackUrl": "http://oss-demo.aliyuncs.com:23450/callback",  # 本地回调服务器地址
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && $_SERVER['REQUEST_URI'] === '/get_post_signature_for_oss_upload') {
    ...
    $result = $request
        ...
        ->withRoleArn(getenv('OSS_STS_ROLE_ARN')) // 替换为您的RAM角色ARN
        ->request();
    // 获取STS请求结果中的凭证信息。
...
| callbackBodyType | 请求发送的Content-Type。 取值： - application/x-www-form-urlencoded（默认值） - application/json |

**说明**

以上仅为回调示例，您可以通过修改服务端代码自行设置回调，请参见[Callback](https://help.aliyun.com/zh/oss/developer-reference/callback)。
...
| dir | 限制上传的文件前缀。 |
| host | Bucket域名。 |
| policy | 用户表单上传的策略（Policy），详情请参见[Post Policy](https://help.aliyun.com/zh/oss/developer-reference/postobject#section-d5z-1ww-wdb)。 |
| security\\_token | 安全令牌。 |
| signature | 对Policy签名后的字符串。详情请参见[Post Signature](https://help.aliyun.com/zh/oss/developer-reference/postobject#section-wny-mww-wdb)。 |
...
如果上传成功，显示“文件已上传”的提示；如果上传失败，显示相应的错误信息。并且在前端页面返回对应的回调信息。

## **结果验证**

完成上述部署后，您可以访问服务端地址，体验Web端签名直传功能。

1.  在浏览器中访问服务端地址，点击选择文件按钮，选择文件后上传。
    
2.  在[Bucket列表](https://oss.console.aliyun.com/bucket)页面，找到存放用户上传文件的Bucket，查看列表确认文件已成功上传
...
2.  选择**释放**。在弹出的下拉菜单中，选择 **实例状态** 区域的 **释放**。
...
3.  在**用户**页面，单击目标RAM用户**操作**列的**删除**。
    ...
    也可以选中多个RAM用户，然后单击用户列表下方的**删除用户**，批量将多个RAM用户移入回收站。
